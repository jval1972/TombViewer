...
uses
  Windows, se_DirectX, Math;

const
  g_PI         =  3.14159265358979323846; // Pi
  g_NEG_PI     = -3.14159265358979323846; // -Pi
  g_2_PI       =  6.28318530717958623200; // 2 * Pi
  g_PI_DIV_2   =  1.57079632679489655800; // Pi / 2
  g_PI_DIV_4   =  0.78539816339744827900; // Pi / 4
  g_PI_DIV_8   =  0.39269908169872415480; // Pi / 8
  g_PI_DIV_16  =  0.19634954084936207740; // Pi / 16
  g_3_PI_DIV_2 =  4.71238898038468985769; // 3*pi/2
  ...
  g_MINHUGE    = -1.0e+38;                // Minimum Huge number for FLOAT
  g_EPSILON    =  1.0e-5;                 // Tolerance for FLOATs

function MakeD3DVector(const x, y, z: TD3DValue): TD3DVector; overload;
function MakeD3DVector(const v: TD3DLVertex): TD3DVector; overload;
...
function MakeD3DVector(const x, y, z: TD3DValue): TD3DVector;
begin
  Result.x := x;
  Result.y := y;
  Result.z := z;
