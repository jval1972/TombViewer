...
uses
  ...
  ExtCtrls, ExtDlgs, ShellAPI, ImgList, Menus, AppEvnts, ComCtrls, StdCtrls,
  Buttons, ToolWin, MessageBox, Tabs,
  se_MyD3DUtils, se_DirectX, se_DXClass, se_DXDraws, se_D3DUtils, se_Main,
  se_DXInput, se_DXDUtils,
  FileMenuHistory, XPMenu, Aboutdlg, AnotherReg,
  zBitmap, xM8, xPPM, xTGA, pngimage, jpeg, filedrag;

type
  TTombRaiderViewerScene = class(TD3DScene)
  public
  ...
  TDXViewerForm = class(TDXForm)
    ...
    FileMenuHistoryItem10: TMenuItem;
    N3: TMenuItem;
    Fog1: TMenuItem;
    Timer1: TTimer;
    ToolButton4: TToolButton;
    EngineSetup2: TSpeedButton;
    DXDrawPanel: TPanel;
    ToolButton6: TToolButton;
    QuickInfo2: TSpeedButton;
    ...
    procedure N3DEngineSetup1Click(Sender: TObject);
  ...
  private
    ...
    regFileMenuHistoryPaths: TStringProfile;
    ...
    regTransparent: TVariantProfile;
    regLightFactor: TVariantProfile;
    regShowFog: TVariantProfile;
    regShowInformation: TVariantProfile;
...
uses
  SyncObjs, Math, Clipbrd, OpenTombRaiderMapFrm, QuickInfoFrm,
  Tomb3DEngineSetupFrm, Variants, trv_globals, se_utils, se_TombRaider;
...
procedure TTombRaiderViewerScene.DrawLoop;
var
  C: TDirectDrawSurfaceCanvas;
  skyverts: integer;
begin
  Inherited;
  if (DXViewerForm.Texture_sky2 <> nil) then
  begin
  ...
  if DXViewerForm.Information1.Checked and (DXViewerForm.Scene.GetNumTriangles > 0) then
  begin
    C := DXDraw.Surface.Canvas;
    C.Brush.Style := bsClear;
    C.Font.Color := clWhite;
    C.TextOut(5, DXDraw.Surface.Height - 48, 'FPS: ' + IntToStr(round(FPS_Smooth)));
    C.TextOut(5, DXDraw.Surface.Height - 32, 'Total triangles: ' + IntToStr(DXViewerForm.Scene.GetNumTriangles + ((320 - 2) div 3)));
...
resourceString
  rsFmtMode = '%dx%d %dbit';
  rsFmtTitleMap = 'TombViewer - %s';
  rsFmtTitle = 'TombViewer';
  rsFmtFileError = 'File %s: %s';
...
function D3DCOLOR(const x: integer): TD3DColor;
begin
  Result := x;
end;
...
begin
  ...
  regFileMenuHistoryPaths := TStringProfile.Create(self);
  regFileMenuHistoryPaths.Key := regFormRestorer1;
  regFileMenuHistoryPaths.Name := 'FileMenuHistoryPaths';

  ErrorNoMapSelectedMessageBox := TMessageBox.Create(self);
  ErrorNoMapSelectedMessageBox.Caption := 'Tomb Raider Viewer';
  ...
  ErrorNoFileSelectMessageBox.Caption := 'Tomb Raider Viewer';
  ...
  ClipboardErrorMessageBox.Caption := 'Tomb Raider Viewer';
  ...
  ErrorMessageBox.Caption := 'Tomb Raider Viewer';
  ...
  MessageBox1.Caption := 'Tomb Raider Viewer';
  ...
  FileDrag1 := TFileDrag.Create(self);
  FileDrag1.EnableDrop := False;
  FileDrag1.OnDrop := FileDrag1Drop;

  FileMenuHistory1 := TFileMenuHistory.Create(self);
  FileMenuHistory1.MenuItem0 := FileMenuHistoryItem1;
  FileMenuHistory1.MenuItem1 := FileMenuHistoryItem2;
  FileMenuHistory1.MenuItem2 := FileMenuHistoryItem3;
  FileMenuHistory1.MenuItem3 := FileMenuHistoryItem4;
  FileMenuHistory1.MenuItem4 := FileMenuHistoryItem5;
  FileMenuHistory1.MenuItem5 := FileMenuHistoryItem6;
  FileMenuHistory1.MenuItem6 := FileMenuHistoryItem7;
  FileMenuHistory1.MenuItem7 := FileMenuHistoryItem8;
  FileMenuHistory1.MenuItem8 := FileMenuHistoryItem9;
  FileMenuHistory1.MenuItem9 := FileMenuHistoryItem10;
...
begin
  ...
  regShowInformation.Free;
  regFileMenuHistoryPaths.Free;
...
begin
  ...
  try
    ...
    Scene := TTombRaiderViewerScene.Create(DXDraw{$IFNDEF NO_D3DSOUNDS}, DXSound{$ENDIF});
    ...
    if not VarIsEmpty(regSafeMode.Value) then
      doSafe := regSafeMode.Value
    else
      doSafe := True;
    SetSafeMode(doSafe, True);

    if not VarIsEmpty(regTransparent.Value) then
      doTransparent := regTransparent.Value
    else
      doTransparent := True;
    ...
    if not VarIsEmpty(regTextureFiltering.Value) then
      ...
      doFiltering := False; // JVAL 20181017 Changed Default Filtering to False

    if not VarIsEmpty(regFileMenuHistoryPaths.Value) then
      with TStringList.Create do
      try
        Text := regFileMenuHistoryPaths.Value;
        for i := 0 to Count - 1 do
          FileMenuHistory1.Paths.Add(Strings[i]);
      finally
        Free;
      end;
    ...
    if not VarIsEmpty(regShowInformation.Value) then
      Information1.Checked := regShowInformation.Value
    else
...
begin
  ...
  regTransparent.Value := doTransparent;
  regTextureFiltering.Value := doFiltering;
  regLightFactor.Value := lFactor;
  regShowFog.Value := Fog1.Checked;
  regShowInformation.Value := Information1.Checked;
  regFileMenuHistoryPaths.Value := FileMenuHistory1.Paths.Text;
...
const
  MAXCULLDISTANCE: single = 10000.0;

function TDXViewerForm.GetCullDistance: TD3DValue;
begin
  if Fog1.Checked then
    result := bCubeSize * 1.5
  else
    result := MAXCULLDISTANCE;
end;
...
var
  ...
  dX, dY, dZ: single;
begin
  if (Time <> oldTime) and // We hope that we get a very very VERY big frame rate!!!!
     Active then
  begin
    { FullScreen movement }
    X := Scene.Position.X;
    ...
    begin
      X := X - vPosition * cos(dY) * (Time - oldTime);
      Z := Z + vPosition * sin(dY) * (Time - oldTime);
    end
    ...
    GetPositionInsideBoundingCube(@bCube, bCubeSize, X, Y, Z);
...
begin
  ...
  try
    ...
    ret := GetTombRaiderLevelData(Scene, MapName, Factor, lFactor, True);
    if ret = 0 then
    begin
      FileMenuHistory1.AddPath(MapName);
      Caption := Format(rsFmtTitleMap, [MkShortName(MapName)]);
      for i := Scene.Surfaces.Count - 1 downto 0 do
        if Scene.Surfaces.Strings[i] = '' then
          Scene.DeleteSurface(i);
      Scene.ReduceMemory;
      ...
      try
        ...
        Texture_sky2 := Scene.AddTextureToCollection('SKY', bmz);
      ...
      btm := Scene.BoundingCube.minY - factor / 64 * 1.5;
      FloorInfo.y1 := btm;
      FloorInfo.y2 := btm;
      FloorInfo.y3 := btm;
      FloorInfo.y4 := btm;
    ...
    begin
      Caption := rsFmtTitle;
      ErrorMessageBox.Execute(Format(rsFmtFileError, [MapName, TombRaiderErrorToString(ret)]));
      MapName := '';
    end;
    AdjustFocus;
    SetSafeMode(Scene.SafeMode);
  finally
    AdjustFog;
    Scene.CullSurfaces(MAXCULLDISTANCE);
    isLoading := False;
    cs.Release;
    cs.Free;
    Screen.Cursor := crDefault;
...
begin
  if QueryImportTombRaiderMap(MapName, lFactor, maxpverts) then
    LoadMap;
end;
...
resourceString
  rsOnlyOneFile = 'Only one file at a time!';

procedure TDXViewerForm.FileDrag1Drop(Sender: TObject);
begin
  if FileDrag1.FileCount = 1 then
  begin
    MapName := FileDrag1.NameWithPath.Strings[0];
    LoadMap;
  end
  else if FileDrag1.FileCount > 1 then
    ErrorMessageBox.Execute(rsOnlyOneFile);
end;

procedure TDXViewerForm.N3DEngineSetup1Click(Sender: TObject);
begin
  if GetTomb3DEngineSetup(self, doUseHardware, doSafe, doTransparent, doFiltering) then
  begin
    SetHardware(doUseHardware);
    SetSafeMode(doSafe);
    AdjustFog;
  end;
end;

procedure TDXViewerForm.File1Click(Sender: TObject);
begin
  FileMenuHistory1.RefreshMenuItems;
end;

procedure TDXViewerForm.FileMenuHistory1Open(Sender: TObject;
  const FileName: TFileName);
begin
  MapName := FileName;
  LoadMap;
...
begin
  ...
  begin
    if Fog1.Checked then
      Scene.StartFog(0, 255, 20.0, 80.0, 0.01)
    else
      Scene.StopFog;
  end;
  dorecalcculls := True;
